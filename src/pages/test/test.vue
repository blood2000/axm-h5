<template>
	<view>
		<button @click="testClick()">发送数据给默认Handler接收</button>
		<button @click="testClick1()">发送数据给指定Handler接收</button>
		<text>打印内容：{{ logContent }}</text>

		<button @click="jump()" style="margin-top: 100rpx;">该页面为临时测试页，点此进统计页面</button>
	</view>
</template>

<script>
	export default {
		data() {
			return {
				logContent: ''
			}
		},
		onLoad(){
			this.initData();
		},
		methods: {
			//发送消息给java代码
			testClick() {
				const _this = this;
				var data = '发送消息给java代码全局接收';
				//第一个参数要发送的数据 第二个参数js在被回调后具体执行方法，responseData为java层回传数据
				window.WebViewJavascriptBridge.send(
					data
					, function(responseData) {
						_this.bridgeLog('来自Java的回传数据： ' + responseData);
					}
				);
			},
			//调用本地java方法
			testClick1() {
				const _this = this;
			    //第一个参数是 调用java的函数名字 第二个参数是要传递的数据 第三个参数js在被回调后具体执行方法，responseData为java层回传数据
			    var data='发送消息给java代码指定接收';
			    window.WebViewJavascriptBridge.callHandler(
				    'submitFromWeb'
				    ,data
				    , function(responseData) {
					    _this.bridgeLog('来自Java的回传数据： ' + responseData);
				    }
			   );
			},
			// 处理数据
			bridgeLog(logContent) {
				this.logContent = logContent;
			},
			//注册事件监听
			connectWebViewJavascriptBridge(callback) {
				if (window.WebViewJavascriptBridge) {
				   callback(WebViewJavascriptBridge)
				} else {
					document.addEventListener(
						'WebViewJavascriptBridgeReady'
						, function() {
						   callback(WebViewJavascriptBridge)
						},
						false
					);
				}
			},
			//注册回调函数，第一次连接时调用 初始化函数
			initData(){
				const _this = this;
				this.connectWebViewJavascriptBridge(function(bridge) {
					bridge.init(function(message, responseCallback) {
						_this.bridgeLog('默认接收收到来自Java数据： ' + message);
						var responseData = '默认接收收到来自Java的数据，回传数据给你';
						responseCallback(responseData);
					});
					bridge.registerHandler("functionInJs", function(data, responseCallback) {
						_this.bridgeLog('指定接收收到来自Java数据： ' + data);
						var responseData = '指定接收收到来自Java的数据，回传数据给你';
						responseCallback(responseData);
					});
				})
			},
			// 跳转到统计
			jump(){
				uni.navigateTo({
					url: '/pages/index/index'
				});
			}
		}
	}
</script>

<style>

</style>
